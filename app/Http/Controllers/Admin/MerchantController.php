...
class MerchantController extends Controller
{
    ...
    public function cancel_collection_merchants(Request $request, Member $member)
    {
        ...
        foreach ($merchant_id as $k => $v) {
            if ($v != $data['merchant_id']) {
                $arr1[] = $v;
            }
        }
        $merchant_id = empty($arr1) ? $arr1 = null : json_encode($arr1);
        $res3 = $member->where('id', $data['member_id'])->update(['merchant_id' => $merchant_id]);
        if ($res3) {
            res(null, '成功');
    ...
    public function cancels_collection_merchants(Request $request, Member $member, Merchant $merchant)
    {
        ...
        $arr = json_encode($arr);
        $res3 = $member->where('id', $data['member_id'])->update(['merchant_id' => $arr]);
        if ($res3) {
            res(null, '成功');
