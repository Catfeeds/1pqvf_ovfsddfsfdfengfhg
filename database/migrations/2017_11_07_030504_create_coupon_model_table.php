...
class CreateCouponModelTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('coupon_category',function(Blueprint $table){
            ...
            $table->unsignedInteger('merchant_id')->nullable()->comment('所属商家');
            $table->string('merchant_name',15)->nullable()->comment('商家名称');
            $table->string('coupon_name',60)->nullable()->comment('优惠券名称');
            $table->unsignedTinyInteger('coupon_type')->nullable()->comment('优惠方式:1金额,2折扣,3扫码(未开通),4提交到兑换,5满减');
            ...
            $table->unsignedInteger('send_num')->nullable()->comment('该券发放的数量');
        ...
        Schema::create('coupon',function(Blueprint $table){
            ...
            $table->unsignedTinyInteger('status')->default(0)->comment('使用状态:0未领取,1已领取但未使用,2已使用,3过期，4冻结期（互换期）');
            ...
            $table->string('cr_adr_code',200)->nullable()->comment('按该区生成的(百度代号adcode),在该区可以查询到');
            $table->unsignedInteger('member_id')->nullable()->comment('所属用户');
            $table->string('province',60)->nullable()->comment('实际所在省');
            $table->string('city',60)->nullable()->comment('实际所在城市');
            $table->string('district',60)->nullable()->comment('实际所在所在区');
            $table->string('adcode',8)->nullable()->comment('优惠券所在区域/百度代号http://lbsyun.baidu.com/index.php?title=open/dev-res');
