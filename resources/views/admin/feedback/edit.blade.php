...
@section('title')编辑反馈@endsection
@section('content')
    <article class="page-container">
            {{-- 隐藏域传递ID --}}
            <form action="{{ url('admin/feedback/' . $feedbackInfo->id ) }}" method="post"  class="form form-horizontal" id="form-comment-edit">
                {{ csrf_field() }}
                {{ method_field('put') }} {{-- 修改一定要加上 表单请求伪造 --}}
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-3">处理：</label>
                    <div class="formControls col-xs-8 col-sm-9">
                        <select name="handling" class="select" id="">
                            <option value="1" @if($feedbackInfo->handling==1) selected @endif>已处理</option>
                            <option value="2" @if($feedbackInfo->handling==2) selected @endif>未处理</option>
                        </select>
                    </div>
                </div>
                <div class="row cl">
