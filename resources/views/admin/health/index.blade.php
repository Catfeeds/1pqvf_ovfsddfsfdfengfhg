...
@section('title')健康达人列表@endsection
@section('content')
{{--主题内容--}}
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 健康达人管理 <span class="c-gray en">&gt;</span>
	健康达人列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<table class="table table-border table-bordered table-bg datatables">
		<thead>
			<tr>
				<th scope="col" colspan="9">健康达人列表</th>
			</tr>
			<tr class="text-c">
				<th width="">ID</th>
				<th width="">用户名</th>
		...
		<tbody>
			<tr class="text-c">
				...
				<td class="td-manage"><a style="text-decoration:none" onClick="health_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
					<a title="编辑" href="javascript:;" onclick="health_edit('健康达人编辑','admin-add.html','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
<script type="text/javascript">
    ...
    $('.datatables').DataTable({
        ...
        "ajax": {
            "url": "{{ url('admin/health/ajax_list') }}",        // 服务端uri地址，显示数据的uri
        ...
        'createdRow':function ( row,data,dataIndex ) {
            ...
            $(row).find('td:eq(3)').html(data.status == 1 ? '完成' : data.status == 2 ? '未完成' : '已领奖');// 使用状态 1:未使用,2:已使用,3:过期
            ...
            $(row).find('td:eq(-2)').html( '<input class="btn radius btn-secondary" onclick="showadd(' +
                '\'收货地址\',\'/admin/health/showadd/'+data.member_id+'\')" type="button" value="查看">' );// 使用状态 1:未使用,2:已使用,3:过期
    ...
    function send(member_id){
        layer.confirm('提示:请先查看用户的收货地址是否正确,发货需谨慎',function(index){
            ...
            url = '/admin/health/send/'+ member_id;
            data = {
                '_token':'{{ csrf_token()  }}',
            };
    ...
    function undo(member_id){
        layer.confirm('提示:请先查看用户的收货地址是否正确,发货需谨慎',function(index){
            //此处请求后台程序，下方是成功后的前台处理……
            url = '/admin/health/undo/'+ member_id;
            data = {
                '_token':'{{ csrf_token()  }}',
            };
            $.get(url,data,function (msg) {
    ...
    function health_del(obj,id){
        layer.confirm('健康达人删除须谨慎，确认要删除吗？',function(index){
            //此处请求后台程序，下方是成功后的前台处理……
            url = '/admin/health/'+ id;
            data = {
                '_token':'{{ csrf_token()  }}',
