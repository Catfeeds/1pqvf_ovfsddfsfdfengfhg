...
@section('title')优惠券图片分类列表@endsection
@section('content')
{{--主题内容--}}
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 优惠券图片分类管理 <span class="c-gray en">&gt;</span>
	优惠券图片分类列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
			<a href="javascript:;" onclick="picture_add('添加优惠券图片','{{ url('admin/picture/create')  }}','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
                添加优惠券图片</a></span>
	</div>
	<table class="table table-border table-bordered table-bg datatables">
		<thead>
			<tr>
				<th scope="col" colspan="7">优惠券图片分类列表</th>
			</tr>
			<tr class="text-c">
				<th width="">ID</th>
				<th width="">商家名称</th>
...
<script type="text/javascript">
    $('.datatables').DataTable({
        ...
        "ajax": {
            "url": "{{ url('admin/picture/ajax_list') }}",
        ...
        'createdRow':function ( row,data,dataIndex ) {
            ...
            $(row).find('td:eq(2)').html(data.action == 1 ? data.price + '元' : data.price + '折');//勾选
            $(row).find('td:eq(-2)').html(data.action == 1 ? '金额' : '折扣');//z状态优惠方式:1=金额.2=折扣.3=扫码(未开通)
            $(row).find('td:eq(3)').html(data.picture_url == null ? '还没有图片' : '<img src="/'+ data.picture_url +'" style="width: 100px;height: 80px;">');
            $(row).find('td:eq(4)').html(data.deduction_url == null ? '还没有图片' : '<img src="/'+ data.deduction_url +'" style="width: 100px;height: 80px;">');

            //操作
            $(row).find('td:eq(-1)').html('<a title="编辑" href="javascript:;" onclick="picture_edit(' +
                '\'优惠券图片分类编辑\',\'/admin/picture/'+data.id+'/edit\',\''+data.id+'\')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> ' +
    ...
    function picture_del(obj,id){
        layer.confirm('优惠券图片分类删除须谨慎，确认要删除吗？',function(index){
