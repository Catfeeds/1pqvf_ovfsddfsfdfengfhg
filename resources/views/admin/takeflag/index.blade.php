...
@section('title')夺旗先锋列表@endsection
@section('content')
{{--主题内容--}}
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 夺旗先锋管理 <span class="c-gray en">&gt;</span>
	夺旗先锋列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">

	<table class="table table-border table-bordered table-bg datatables">
		<thead>
			<tr>
				<th scope="col" colspan="9">夺旗先锋列表</th>
			</tr>
			<tr class="text-c">
				<th width="">ID</th>
				<th width="">用户名</th>
		...
		<tbody>
			<tr class="text-c">
				...
				<td>用户名</td>
				<td>备注</td>
				<td class="td-manage"><a style="text-decoration:none" onClick="health_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
					<a title="编辑" href="javascript:;" onclick="health_edit('夺旗先锋编辑','admin-add.html','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
					<a title="删除" href="javascript:;" onclick="health_del(this,'1')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
...
<script type="text/javascript">
    ...
    $('.datatables').DataTable({
        ...
        "ajax": {
            "url": "{{ url('admin/takeflag/ajax_list') }}",
        ...
        'createdRow':function ( row,data,dataIndex ) {
            ...
            $(row).find('td:eq(3)').html(data.status == 1 ? '完成' : data.status == 2 ? '未完成' : '已领奖');
            $(row).find('td:eq(4)').html(data.award == null ? '未领奖' :data.award);
            $(row).find('td:eq(5)').html(data.delivery == null ? '未发货' :'已发货');
            $(row).find('td:eq(-2)').html( '<input class="btn radius btn-secondary" onclick="showadd(' +
                '\'收货地址\',\'/admin/takeflag/showadd/'+data.member_id+'\')" type="button" value="查看">' );
            //操作
    ...
    function send(member_id){
        layer.confirm('提示:请先查看用户的收货地址是否正确,发货需谨慎',function(index){
            ...
            url = '/admin/takeflag/send/'+ member_id;
            data = {
                '_token':'{{ csrf_token()  }}',
            };
    ...
    function undo(member_id){
        layer.confirm('提示:请先查看用户的收货地址是否正确,发货需谨慎',function(index){
            url = '/admin/takeflag/undo/'+ member_id;
            data = {
                '_token':'{{ csrf_token()  }}',
            };
            $.get(url,data,function (msg) {
	...
	/*夺旗先锋-夺旗先锋-添加*/
    function health_add(title,url,w,h){
        layer_show(title,url,'1200','800');
    }
	/*夺旗先锋-夺旗先锋-编辑*/
    function health_edit(title,url,id,w,h){
        layer_show(title,url,'1200','800');
    }
	/*夺旗先锋-夺旗先锋-删除*/
    function health_del(obj,id){
        layer.confirm('夺旗先锋删除须谨慎，确认要删除吗？',function(index){
            url = '/admin/takeflag/'+ id;
            data = {
                '_token':'{{ csrf_token()  }}',
                '_method':'delete',
            };
