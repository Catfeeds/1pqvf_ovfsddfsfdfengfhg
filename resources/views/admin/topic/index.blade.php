    ...
    $('.datatables').DataTable({
        ...
        'createdRow':function ( row,data,dataIndex ) {
            ...
            if(data.img_url == null){
                $(row).find('td:eq(5)').html('暂无图片');
            }else{
                //先将img分割成数组
                var arr = new Array();
                var img = JSON.stringify(data.img_url);//对象转字符串
                arr = img.split(',');
            ...
            for(var i=0;i<content.length;i++){
                data.content = content.replace('\'','ct2rs1')
                data.content = content.replace('\"','ct2rs2')
                data.content = content.replace('&','ct2rs3')
            }
            $(row).find('td:eq(4)').html('<input class="btn radius btn-secondary" onclick="showContent(\' '+data.content+' \')" type="button" value="查看">');//z状态
            $(row).find('td:eq(2)').html(data.nice_num);//点赞数
            //操作
